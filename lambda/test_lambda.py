...
class TestLambdaLifecycle(unittest.TestCase):
    def test_should_delete_all_accessed(self):
        item = {
            "file_id": "fid-1",
    ...
    def test_should_delete_pending_retained(self):
        item = {
            "file_id": "fid-2",
            "recipients": ["alice@test.com", "bob@test.com"],
    ...
    def test_should_delete_hard_expiry(self):
    ...
    def test_lambda_handler_execution(self, mock_get_dynamo, mock_get_s3):
        mock_s3 = MagicMock()
        ...
        mock_s3.delete_object.assert_called_once_with(
            Bucket="cloudshare-secure-bucket",
            Key="uploads/fid-completed/file.pdf"
        )
        mock_table.delete_item.assert_called_once_with(Key={"file_id": "fid-completed"})
